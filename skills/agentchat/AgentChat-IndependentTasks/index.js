...
// v27: route calls through the shared worker pool (warm playwright-core + CDP
// connection) when enabled. Any pool failure falls back to spawn inside
// execute.js — this is a pure acceleration, never a new failure class.
  ...
  if (perCallCapMs !== null) {
    ...
    if (Number.isFinite(perCallCapMs) && perCallCapMs > 0) {
      const exec = createExecutor({
        ...
        perCallCapMs,
        workerPool, // v27: same shared pool, so per-call mode reuses warm workers too
      });
