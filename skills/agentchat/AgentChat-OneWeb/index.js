...
// ══════════════════════════════════════════════════════════════════════════════
// PROVIDER RUNNERS — LAZY factory-built from adapter configs in lib/providers/adapters/
// ══════════════════════════════════════════════════════════════════════════════
...
//
// v27 (perf): adapters are now loaded ON DEMAND instead of all 10 at startup.
// A single-provider call (--only/--from --single) previously paid the require
// cost of every adapter module (~10 files + their deps) even though only one
// ever ran; a cold fallback chain paid all 10 up front. getRunner() caches the
// factory result so each adapter is required exactly once per process, and
// only when the chain actually reaches it. Saves ~90% of module loading on
// single-provider runs and keeps worker-pool processes (see lib/worker-pool.js)
// lean until a provider is genuinely tried.
...
function getRunner(key) {
    let runner = RUNNER_CACHE.get(key);
    if (runner) return runner;
    const cfg = require(`../lib/providers/adapters/${key}`);
    // Gemini uses its own spinner-free runner; all others share the progress spinner
    runner = createProviderRunner(key === 'gemini' ? cfg : { ...cfg, onProgress: spinner });
...
async function tryAllProviders(browser, prompt, ctx, options = {}) {
    ...
    for (let i = startIdx; i < endIdx; i++) {
        ...
        try {
            ...
            const runner = getRunner(provider.key);
            result = runner
                ? await runner(page, prompt, perProvTimeout, ctx, {
                      images: options.images,
                      // v27: streaming-delta sink threads through to
                      // waitForCompletion's onStream (see executeOnce).
...
async function main() {
    ...
    let streamEnabled = true;  // v27: streaming-delta stderr lines (--no-stream to disable)
    ...
    const USAGE =
        'Usage: node index.js [--timeout=MS] [--from=NAME] [--only=NAME] [--single] [--image] [--image-path=PATH] [--locale=xx_XX] [--keep-tabs] [--close] [--ephemeral-tab] [--no-download-images] [--no-stream] [--smoke] [--doctor] "Your prompt"\n' +
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (a === '--smoke') {
            ...
            // v27: streaming-delta opt-out — suppress [stream] stderr lines
            // (long generations then show only the final answer, as pre-v27).
            streamEnabled = false;
    ...
    try {
        ...
        // v27: execution core extracted to executeOnce() so worker-pool
        // processes (lib/worker-entry.js) can reuse a warm CDP connection
        // instead of re-connecting per call. Same pipeline, same telemetry,
        // same exit-code semantics — only the exit path differs (return vs
        // process.exit).
        const outcome = await executeOnce(browser, prompt, ctx, {
            ...
            downloadImages,
            cwd: process.cwd(),
            stream: streamEnabled, // v27: --no-stream disables [stream] lines
        });

        // ── stdout delivery (P0 FLUSH FIX preserved) ──
        // console.log + immediate process.exit truncates piped stdout at the
        // pipe-buffer boundary (~128KB on Linux). Exit only from the write
        // callback after the kernel accepted the full payload. (process.exit
        // is still required here — the CDP websocket keeps the event loop
        // alive, so a natural exit never happens.)
        if (outcome.stdout != null) {
            process.stdout.once('error', () => process.exit(outcome.exitCode));
...
// ══════════════════════════════════════════════════════════════════════════════
// v27: PROGRAMMATIC EXECUTION CORE — shared by the CLI entry (main) and the
// worker pool (lib/worker-entry.js). Runs the full fallback-chain pipeline on
// a browser connection and returns a structured outcome instead of exiting.
// The CLI wrapper delivers stdout + exit code; worker processes deliver the
// same via IPC, reusing one warm CDP connection across many calls.
// ══════════════════════════════════════════════════════════════════════════════

/**
 * Run the fallback chain on an existing browser connection.
 * @param {object} browser - connected playwright browser (CDP guest session)
 * @param {string} prompt - the user prompt to send
 * @param {InvocationContext} ctx - per-run telemetry/receipt state
 * @param {object} [opts]
 * @param {number} [opts.totalTimeout] - wall-clock budget across the chain
 * @param {number|null} [opts.providerTimeout] - per-provider budget
 * @param {string|null} [opts.startFrom] - first provider to try
 * @param {boolean} [opts.keepTabs] - keep self-created tabs open on success
 * @param {boolean} [opts.singleAttempt] - try exactly one provider, no cascade
 * @param {boolean} [opts.ephemeralTab] - dedicated tab, never reused, closed on exit
 * @param {Array} [opts.images] - pre-encoded images to paste before the prompt
 * @param {boolean} [opts.downloadImages] - auto-download images from the response
 * @param {string} [opts.cwd] - download target directory (defaults to process.cwd())
 * @param {boolean} [opts.stream] - enable the streaming-delta channel on stderr
 *        (default true). Line format: `[stream] {"provider":..,"chars":..,"delta":..,"ms":..}`
 * @param {(obj: object) => void} [opts.onStream] - custom streaming sink; when
 *        given, replaces the default stderr emitter (stream flag ignored).
 * @returns {Promise<{exitCode: number, stdout: string|null}>} stdout is null on
 *          failure (structured failure already emitted to stderr).
 */
async function executeOnce(browser, prompt, ctx, {
    ...
    // v27: STREAMING SINK — default writes a machine-parseable stderr line per
    // growth delta; a caller-supplied onStream replaces it (e.g. worker entry
    // forwarding deltas over IPC, or a UI rendering progress). Prefix mirrors
    // the receipt pattern: `[stream] {json}` on stderr, stdout stays clean.
    const streamSink = (onStream != null)
        ? onStream
        : (stream
            ? (obj) => {
            ...
            : null);
    // ── v17: browser-level admission control (see main() for rationale) ──
    let browserSlot = null;
    try {
        const slotWait = Math.min(60_000, Math.floor(totalTimeout * 0.25));
        ...
        try {
            result = await tryAllProviders(browser, prompt, ctx, {
                ...
                images, // v24: images to paste before prompt text
                onStream: streamSink, // v27: streaming-delta channel
        ...
        if (result.success) {
            ...
            if (downloadImages) {
                try {
                    const dlResult = await downloadAllImages(result.response, cwd, { page: result.page }); // v13: session-aware download
            ...
            return { exitCode: 0, stdout: finalResponse };
        }

        // v23: STRUCTURED ERROR CHANNEL (see main() for rationale)
        ...
        if (hasTotalTimeout) {
            ...
            return { exitCode: 10, stdout: null };
        }
        ...
        return { exitCode: 9, stdout: null };
    } catch (err) {
        log(`FATAL: ${err.message}`);
        emitStructuredFailure({ __fatal: { reason: 'internal', error_details: { stage: 'unhandled', message: err.message } } });
        ctx.recordTelemetry(4);
        return { exitCode: 4, stdout: null };
    } finally {
        if (browserSlot !== null) releaseBrowserSlot(browserSlot);
        // POLICY: NEVER call browser.close() — this is a CDP guest session.
        // Closing the browser destroys ALL the user's tabs, not just ours.
    }
}
...
module.exports = {
    ...
    fetchViaBrowser,
    sniffImageExt,
    // v27: programmatic execution core — reused by lib/worker-entry.js so
    // worker-pool processes can share one warm CDP connection instead of
    // re-connecting per call.
    executeOnce,
    // InvocationContext is exported for worker entry to construct per-run
    // telemetry state identically to the CLI path.
    InvocationContext,
    // smokeTest is exported so workers can answer --smoke without a fork.
    smokeTest,
};
