...
description: Multi-provider CDP bridge with automatic fallback (Gemini->ChatGPT->Claude->Qwen->Kimi->MiniMax->Doubao->MiMo->DeepSeek). Use for AI provider failover, fallback chain, multi-provider routing, or "send to any available AI". MANDATORY EXECUTION - invoking this skill REQUIRES running `node $AGENTCHAT_SKILL_DIR/AgentChat-OneWeb/index.js "<prompt>"` as the FIRST action and quoting its `[receipt] AGENTCHAT_RUN` line in the final answer; explaining the skill or answering from model knowledge without a receipt is a violation.
---
...
```bash
node $AGENTCHAT_SKILL_DIR/AgentChat-OneWeb/index.js "<用户prompt>"
```
...
- `run_id` 为随机生成并同步落盘到 `$AGENTCHAT_SKILL_DIR/AgentChat-OneWeb/data/receipts.jsonl`，用户可用 `grep <run_id>` 核对——凭空编造无法通过核对。
...
```bash
node $AGENTCHAT_SKILL_DIR/AgentChat-OneWeb/index.js --image "<用户原始prompt>"
```
...
```bash
node $AGENTCHAT_SKILL_DIR/AgentChat-OneWeb/index.js --image-path="<图片绝对路径>" "<用户prompt>"
```
...
```bash
# 单张图片
node $AGENTCHAT_SKILL_DIR/AgentChat-OneWeb/index.js --image-path=./screenshot.png "分析这张截图的内容"

# 多张图片
node $AGENTCHAT_SKILL_DIR/AgentChat-OneWeb/index.js \
...
4. **执行命令**：运行 `node $AGENTCHAT_SKILL_DIR/AgentChat-OneWeb/index.js --image-path=... "prompt"`
...
node $AGENTCHAT_SKILL_DIR/AgentChat-OneWeb/index.js "Your prompt"

# 执行完毕后自动清理浏览器标签
node $AGENTCHAT_SKILL_DIR/AgentChat-OneWeb/index.js --close "Your prompt"

# 指定超时 (ms)
node $AGENTCHAT_SKILL_DIR/AgentChat-OneWeb/index.js --timeout=600000 "Long prompt..."

# 从 stdin 读取
echo "Prompt from pipe" | node $AGENTCHAT_SKILL_DIR/AgentChat-OneWeb/index.js

# 环境检查 (不发送 prompt)
node $AGENTCHAT_SKILL_DIR/AgentChat-OneWeb/index.js --smoke

# CDP 连通性检查
node $AGENTCHAT_SKILL_DIR/AgentChat-OneWeb/index.js --doctor

# 强制指定起始 provider (跳过前面的)
node $AGENTCHAT_SKILL_DIR/AgentChat-OneWeb/index.js --from=ChatGPT "prompt"

# 上传图片并提问（可重复 --image-path 上传多张）
node $AGENTCHAT_SKILL_DIR/AgentChat-OneWeb/index.js --image-path=./photo.png "描述这张图片"
node $AGENTCHAT_SKILL_DIR/AgentChat-OneWeb/index.js --image-path=a.png --image-path=b.jpg "比较这两张图片"
```
...
| `--no-stream` | 禁用流式增量输出（默认启用：生成过程中 stderr 输出 `[stream] {json}` 行，含 provider/chars/delta/ms，约每 3s 一条；长生成（Gemini Pro Extended 3-5 分钟）不再静默） |
...
- **telemetry**: 写入 `$AGENTCHAT_SKILL_DIR/AgentChat-OneWeb/data/fallback-telemetry.jsonl`
