...
function createExecutor({
    ...
    workerPool = null, // v27: optional shared WorkerPool; calls route through it first
} = {}) {
    ...
    function callProvider(prompt, provider, timeoutMs, callOpts = {}) {
        ...
        return callViaPoolOrSpawn();

        // ── v27: route through the worker pool first, spawn as fallback ──
        async function callViaPoolOrSpawn() {

        // v27: WORKER-POOL FAST PATH. When a shared WorkerPool was injected,
        // route the call through a warm worker (reuses playwright-core + CDP
        // connection) instead of spawning a fresh node process. The pool is an
        // acceleration layer with a hard contract: ANY pool failure (unhealthy,
        // worker crash, queue timeout, IPC error) rejects here and falls
        // through to the battle-tested spawn path below — never a new failure
        // class. The worker executes the identical OneWeb pipeline (--only +
        // --single semantics via executeOnce), so the outcome shape matches the
        // spawn contract: {exitCode, stdout} with the structured-error line
        // already emitted to stderr by the worker.
        if (workerPool) {
            const poolAttempt = workerPool.execute({
                prompt: String(prompt),
                provider,
                timeoutMs,
                ephemeralTab: !!callOpts.ephemeralTab,
            }).then((outcome) => {
                const code = outcome && outcome.exitCode;
                const text = (outcome && outcome.stdout) || "";
                if (code === 0 && text.length >= 5) {
                    return { ok: true, text, provider };
                }
                // Map the worker's exit code to the same coarse reason the
                // spawn path would derive. workerId rides along for diagnostics.
                const reason = code != null ? (EXIT_REASONS[code] || `exit_${code}`) : "pool_error";
                return { ok: false, text: "", provider, reason, ...(outcome && outcome.workerId ? { workerId: outcome.workerId } : {}) };
            });
            // Pool unavailable/crashed → degrade to spawn, never fail here.
            const decided = await poolAttempt.catch((err) => {
                log(`worker pool unavailable (${err.message}) — falling back to spawn`);
                return null;
            });
        ...
        // ── spawn fallback path (original implementation) ──
        function spawnCall() {
